...
class GoblinImgGenerate:
    def __init__(self):
        self.root_file = get_config("image", "goblin")
        self.part = get_config("image", "goblin_parts")
    ...
    def _get_parts_list(self):
        ...
        parts_choice_order = dict(
            sorted(body_parts.items(), key=lambda item: int(item[0]))
        )
        temp_list = [
